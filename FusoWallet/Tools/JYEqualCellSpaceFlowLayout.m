...
-(instancetype)init{
    return [self initWithType:AlignWithCenter betweenOfCell:8.0];
}
...
-(instancetype)initWithType:(AlignType)cellType betweenOfCell:(CGFloat)betweenOfCell{
    ...
    if (self){
        ...
        self.minimumInteritemSpacing = betweenOfCell;
        self.sectionInset = UIEdgeInsetsMake(12, 24, 8, 24);
        _betweenOfCell = betweenOfCell;
        _cellType = cellType;
    }
    return self;
...
-(void)setCellFrameWith:(NSMutableArray*)layoutAttributes{
    ...
    switch (_cellType) {
        ...
        case AlignWithCenter:
            nowWidth = (self.collectionView.frame.size.width - _sumCellWidth - ((layoutAttributes.count - 1) * _betweenOfCell)) / 2.f;
            for (UICollectionViewLayoutAttributes * attributes in layoutAttributes) {
                CGRect nowFrame = attributes.frame;
                nowFrame.origin.x = nowWidth;
                attributes.frame = nowFrame;
